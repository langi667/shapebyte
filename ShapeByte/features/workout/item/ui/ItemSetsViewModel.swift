...
open class ItemSetsViewModel: ViewModel, ObservableObject {
    ...
    func handleSetsStateReceived( _ state: ItemSetsState ) -> Bool {
        return true
    }
    ...
    private func handleCoordinatorStateChanged(_ state: ItemSetsState) {
        if handleSetsStateReceived(state) {
            self.state = state
        }
